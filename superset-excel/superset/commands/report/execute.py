...
class BaseReportState:
    ...
    def _get_xlsx_data(self) -> bytes:
        """
        Get chart data as XLSX (Excel) format
        ...
        try:
            # 复用 _get_embedded_data 获取 DataFrame
            dataframe = self._get_embedded_data()
            # 使用 Superset 内置的 df_to_excel 转换
            xlsx_data = df_to_excel(dataframe)
            
            elapsed_seconds = (datetime.utcnow() - start_time).total_seconds()
            logger.info(
                "XLSX data generation as user %s took %.2fs - execution_id: %s",
                username,
                elapsed_seconds,
                self._execution_id,
            )
