...
class ExtraCache:
    ...
    regex = re.compile(
        ...
        r"current_user_groups_name\([^)]*\)|"
        r"current_user_groups_lable\([^)]*\)|"
        r"cache_key_wrapper\([^)]*\)|"
        r"url_param\([^)]*\)"
        r")"
        r"[^{}]*?(\}\}|\%\})"
    ...
    def current_user_groups_name(self, index: int = -1, index_str:str = ' ', add_to_cache_keys: bool = True) -> list[str] | None:
        """
        Return the sorted list of groups of the user who is currently logged in.

        :param add_to_cache_keys: Whether the value should be included in the cache key
        :returns: List of role names
        """
        if g.user and not g.user.is_anonymous and hasattr(g.user, 'groups'):
            try:
                user_groups = sorted(
                    [group.name for group in g.user.groups]
                )
                if index >= 0:
                    retun_list = []
                    for group in g.user.groups:
                        # 如果比較短時就取最後一個
                        if len(group.name.split(index_str)) < index + 1:
                            retun_list.append(group.name.split(index_str)[-1])
                        else:
                            retun_list.append(group.name.split(index_str)[index])
                    user_groups = sorted(retun_list)
                if not user_groups:
                    return None
                if add_to_cache_keys:
                    self.cache_key_wrapper(json.dumps(user_groups))
                return user_groups
            except Exception:  # pylint: disable=broad-except
                return None
        return None
    
    def current_user_groups_label(self, index: int = -1, index_str:str = ' ', add_to_cache_keys: bool = True) -> list[str] | None:
        """
        Return the sorted list of groups of the user who is currently logged in.
        ...
        if g.user and not g.user.is_anonymous and hasattr(g.user, 'groups'):
            try:
                user_groups = sorted(
                    [group.label for group in g.user.groups]
                )
                if index >= 0:
                    retun_list = []
                    for group in g.user.groups:
                        # 如果比較短時就取最後一個
                        if len(group.label.split(index_str)) < index + 1:
                            retun_list.append(group.label.split(index_str)[-1])
                        else:
                            retun_list.append(group.label.split(index_str)[index])
                    user_groups = sorted(retun_list)
                if not user_groups:
                    return None
                if add_to_cache_keys:
        ...
        self._context.update(
            {
                ...
                "current_user_groups_name": partial(
                    safe_proxy, extra_cache.current_user_groups_name
                ),                
                "current_user_groups_label": partial(
                    safe_proxy, extra_cache.current_user_groups_label
                ),                
